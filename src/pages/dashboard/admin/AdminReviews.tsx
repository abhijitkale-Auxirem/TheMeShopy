...
import { Star, Search, CheckCircle, XCircle, ThumbsUp, Eye, Flag } from 'lucide-react';
import { useState } from 'react';
import { mockReviews } from '@/database/mockDb';

const adminReviews = mockReviews.map((r, i) => ({
  ...r,
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
          {[
            ...
            { label: 'Approved', value: adminReviews.filter(r => r.status === 'approved').length },
            { label: 'Flagged', value: adminReviews.filter(r => r.status === 'flagged').length },
            { label: 'Avg Rating', value: (adminReviews.reduce((s, r) => s + r.rating, 0) / adminReviews.length).toFixed(1) },
          ].map(s => (
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-start justify-between flex-wrap gap-2 mb-2">
                    ...
                    <div className="flex gap-2">
                      <span className={`text-xs font-medium px-2 py-0.5 rounded-full ${review.status === 'approved' ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400' : 'bg-red-100 text-red-600 dark:bg-red-900/30 dark:text-red-400'}`}>{review.status}</span>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center justify-between mt-3">
                    ...
                    <div className="flex gap-2">
                      <button className="p-1.5 text-gray-400 hover:text-emerald-600 hover:bg-emerald-50 dark:hover:bg-emerald-900/20 rounded-lg" title="Approve"><CheckCircle className="w-4 h-4" /></button>
                      <button className="p-1.5 text-gray-400 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg" title="Remove"><XCircle className="w-4 h-4" /></button>
                    </div>
                  </div>
                </div>
              </div>
